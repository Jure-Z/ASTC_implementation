...
static bool generate_one_partition_info_entry(
	...
	int texels_per_block = block_descriptor.uniform_variables.texel_count;
	...
	for (unsigned int y = 0; y < block_descriptor.uniform_variables.ydim; y++)
	{
	...
	if (partition_count == 2)
	{
		bitmaps = block_descriptor.coverage_bitmaps_2[partition_remap_index];
	}
	else if (partition_count == 3)
	{
		bitmaps = block_descriptor.coverage_bitmaps_3[partition_remap_index];
	}
	else if (partition_count == 4)
	{
		bitmaps = block_descriptor.coverage_bitmaps_4[partition_remap_index];
	}

	// Populate partition texel counts
	for (unsigned int i = 0; i < BLOCK_MAX_PARTITIONS; i++)
	{
...
static void build_partition_table_for_one_partition_count(
	...
	block_descriptor.partitioning_count_selected[partition_count - 1] = 0;
	block_descriptor.partitioning_count_all[partition_count - 1] = 0;
	...
	for (unsigned int x = 0; x < max_iter; x++)
	{
		for (unsigned int i = 0; i < BLOCK_MAX_PARTITIONINGS; i++)
		{
			...
			bool keep_useful = generate_one_partition_info_entry(block_descriptor, partition_count, i, next_index, ptab[next_index]);
			if ((x == 0) && !keep_useful)
			{
				continue;
			}

			generate_canonical_partitioning(block_descriptor.uniform_variables.texel_count, ptab[next_index].partition_of_texel, canonical_patterns + next_index * BIT_PATTERN_WORDS);
			...
			if (keep_useful && keep_canonical)
			{
				if (x == 0)
				{
					block_descriptor.partitioning_packed_index[partition_count - 2][i] = static_cast<uint16_t>(next_index);
					block_descriptor.partitioning_count_selected[partition_count - 1]++;
					block_descriptor.partitioning_count_all[partition_count - 1]++;
					build[i] = 1;
					next_index++;
				}
			}
...
void init_partition_tables(
	...
	partition_info* par_tab2 = block_descriptor.partitionings;
	...
	generate_one_partition_info_entry(block_descriptor, 1, 0, 0, *par_tab1);
	block_descriptor.partitioning_count_selected[0] = 1;
	block_descriptor.partitioning_count_all[0] = 1;

	uint64_t* canonical_patterns = new uint64_t[BLOCK_MAX_PARTITIONINGS * BIT_PATTERN_WORDS];

	build_partition_table_for_one_partition_count(block_descriptor, can_omit_partitionings, partition_count_cutoff, 2, par_tab2, canonical_patterns);
	build_partition_table_for_one_partition_count(block_descriptor, can_omit_partitionings, partition_count_cutoff, 3, par_tab3, canonical_patterns);
	build_partition_table_for_one_partition_count(block_descriptor, can_omit_partitionings, partition_count_cutoff, 4, par_tab4, canonical_patterns);
